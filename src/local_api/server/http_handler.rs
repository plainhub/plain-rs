...
use crate::{base64_decode, base64_encode, xchacha_decrypt, xchacha_encrypt};
    ...
    W: AsyncWrite + Unpin,
{
    ...
    if dlna::is_receiver_path(&method, &path) {
        if !ctx.shell.dlna_enabled() || !ctx.dlna_engine.is_running() {
        ...
        let allowed = ctx.shell.dlna_senders("dlna_allowed_senders");
        let denied = ctx.shell.dlna_senders("dlna_denied_senders");
        ...
        ("POST", "/init") => {
            ...
            if content_length > 0 && !ctx.token.is_empty() {
                ...
                if reader.read_exact(&mut body).await.is_ok() {
                    authenticated = crate::xchacha_decrypt(&ctx.token, &body).is_some();
                }
            }
