...
//!
//! Returns `(width, height)` for JPEG / PNG / GIF / BMP / WebP / ICO by
//! reading the format's magic bytes and header fields directly — mirrors
//! `plain-app`'s `platform/getImageDimensions` (Android `BitmapFactory`
//! behaviour, including ICO where a `0` entry encodes 256). This is the
//! single shared parser used by the local `fileInfo` resolver and by
//! async link-preview image validation. Anything undecodable yields `None`.
